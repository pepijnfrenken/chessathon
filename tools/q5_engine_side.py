...
sys.path.insert(0, sys.argv[1] if len(sys.argv) > 1 else os.path.dirname(
...
def _warmup() -> None:
    # q5-fix1 bout copy: cover the REAL move shape — deep root iteration
    # with non-empty game history — so the first clocked move does not pay
    # late-JIT compile cost. (tools/engine_side.py keeps the shallow probe;
    # its 500ms gates never noticed, real-clock flagging did.)
...
def main() -> int:
    ...
    for line in sys.stdin:
        ...
        if not fen or fen == "quit":
            break
        if fen.startswith("budget "):
            _PENDING_BUDGET_MS = int(fen.split()[1])
            continue
        if fen == "reset":
